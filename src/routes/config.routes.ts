...
import type { IBotComponent } from "@/types/models";
import { postConfigValidator } from "@/validators/config";
import { Elysia } from "elysia";

const characterCounter = (botComponent: IBotComponent) => {
    ...
    embeds?.forEach((embed) => {
        ...
        const fieldsLength = fields?.reduce((acc, field) => {
        ...
        if (totalLength > 6000) {
            throw new Error("The combined sum of all characters in the title, description, footer.text, author.name, fields.name, and fields.value must be less than or equal to 6000 characters.");
        }
    });
...
.post("/", saveConfig, {
    beforeHandle: (context) => {
        return characterCounter(context.body.initialMessage as IBotComponent);
    }
});
