...
export const drop: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("message")
        .setDescription("The message to announce for the drop")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("claim-word")
        .setDescription("The word to claim the drop")
        .setRequired(true)
    )
    ...
    .addRoleOption((option) =>
      option
        .setName("winner-role")
        .setDescription("Role that should be given to the winners.")
    )
    .addIntegerOption((option) =>
      option
        .setName("winner-count")
        .setDescription("The number of winners, defaults to 1")
    )
    .addStringOption((option) =>
      option
        .setName("prizes")
        .setDescription("Comma-separated list of prizes, e.g., '10,20,30'")
    )
    .addStringOption((option) =>
      option
        .setName("victory-message")
        .setDescription(
          "Custom victory message. Use {winner} and {prize} placeholders."
        )
    )
    .addStringOption((option) =>
      option
        .setName("end-message")
        .setDescription("The message if no one wins the drop")
    )
    .addStringOption((option) =>
      option
        .setName("time")
        .setDescription("Duration of the drop, defaults to 60m")
    )
    .addBooleanOption((option) =>
      option
        .setName("case-sensitive")
        .setDescription("Should the claim word be case sensitive?")
    )
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const victoryMessage =
      interaction.options.getString("victory-message") ||
      "Congratulations {winner}, you won {prize} points!";
    const endMessage =
      interaction.options.getString("end-message") || "No one won the drop!";
    const time = interaction.options.getString("time") || "60m";
    const caseSensitive =
      interaction.options.getBoolean("case-sensitive") || false;
    const winnerRole = interaction.options.getRole("winner-role");

    const durationInMs = ms(time);
    const channel =
      (channelOption as BaseGuildTextChannel) || interaction.channel;

    const prizeArray = prizesInput
      .split(",")
    ...
    collector.on("end", async (collected, _reason) => {
      ...
      const winnersWithPrizes = collectedWinners.length
        ? collectedWinners
            .map(
              (w, i) =>
                (collectedWinners.length > 1 ? `${i + 1}.` : "").toString() +
                `<@${w.userId}> got ${w.prize} points`
            )
            .join("\n")
        : endMessage;
      if (logChannel) {
